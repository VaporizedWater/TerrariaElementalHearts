    ...
    internal class EHR_RippleEffect : ModProjectile {
        ...
        public override void AI() {
            if (Projectile.ai[0] == 1.0) {
                Projectile.ai[0] = 2f;
                if (Main.netMode != NetmodeID.Server && !Filters.Scene[Constants.RippleEffectName].IsActive())
                    Filters.Scene.Activate(Constants.RippleEffectName, Projectile.Center, []).GetShader().UseColor(rippleCount, rippleSize, rippleSpeed).UseTargetPosition(Projectile.Center);
            }
            if (Main.netMode == NetmodeID.Server || !Filters.Scene[Constants.RippleEffectName].IsActive())
                return;
            float num = (333 - Projectile.timeLeft) / 90f;
            Filters.Scene[Constants.RippleEffectName].GetShader().UseProgress(num).UseOpacity(distortStrength * (float)(1.0 - (double)num / 3.0));
        }

        public override void OnKill(int timeLeft) {
            if (Main.netMode == NetmodeID.Server || !Filters.Scene[Constants.RippleEffectName].IsActive())
                return;
            Filters.Scene[Constants.RippleEffectName].Deactivate([]);
        }
    }
}
